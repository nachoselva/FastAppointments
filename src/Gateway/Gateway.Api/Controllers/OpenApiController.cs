namespace Gateway.Api.Controllers
{
        ...
        IMemoryCache cache) : ControllerBase
    {
        ...
        {
            if (!cache.TryGetValue(OPEN_API_CACHE_KEY, out OpenApiDocument? mergedDocument) || mergedDocument == null)
            {
                mergedDocument = await BuildOpenApiDocument();

                cache.Set(OPEN_API_CACHE_KEY, mergedDocument, new MemoryCacheEntryOptions
                {
        ...
        private async Task<OpenApiDocument> BuildOpenApiDocument()
        {
            ...
            var apiConfigs = configuration.GetApisConfig();
